...
(()=>{
 ...
 function previewConflicts(){
  ...
  for(const c of overlaps){const item=document.createElement('li');item.textContent=data.athletes.find(a=>a.id===c.athleteId).name+': '+[...c.blocks.map(b=>b.name+' (block '+VXT.formatDate(b.startDate)+' – '+VXT.formatDate(b.endDate)+')'),...c.workouts.map(w=>w.name+' (team workout, '+VXT.formatStartTime(w.startTime)+')'),...c.sessions.map(s=>s.name+' (saved session, '+VXT.formatStartTime(s.startTime)+')')].join('; ');el('team-conflict-list').append(item);if(globalThis.VXTUX)VXTUX.conflictActions(item,c,()=>{for(const input of el('team-athletes').querySelectorAll('input'))if(input.value===c.athleteId)input.checked=false;count();document.dispatchEvent(new CustomEvent('vxt-draft-change',{detail:'team'}));});}
 }
 ...
 function reset(){document.dispatchEvent(new CustomEvent('vxt-clear-draft',{detail:'team'}));editing=null;el('team-form').reset();el('team-date').value=today();el('team-event').value='60m';roster([]);el('team-save').textContent='Assign workout';el('team-cancel').hidden=true;}
 function history(openId){
  ...
  for(const w of [...data.teamWorkouts].sort((a,b)=>b.date.localeCompare(a.date))){
   ...
   const practice=document.createElement('button');practice.type='button';practice.textContent='Start practice for '+w.name;practice.addEventListener('click',()=>document.dispatchEvent(new CustomEvent('vxt-start-practice',{detail:w.id})));section.append(practice);
 ...
 (globalThis.VXTEditors ||= {}).team={form:'team-form',capture(){return {editing,fields:Object.fromEntries(['team-name','team-date','team-start-time','team-workout','team-notes','team-event','team-reps','team-rest'].map(id=>[id,el(id).value])),athleteIds:selected(),targets:globalThis.VXTUX?.targets()||{}};},restore(d){
  if(!d.fields||!Array.isArray(d.athleteIds))throw Error('Workout draft is invalid.');if(d.editing&&!VXTLocal.snapshot().teamWorkouts?.some(w=>w.id===d.editing))throw Error('Edited workout no longer exists. Discard this draft or undo its deletion.');
  editing=d.editing||null;for(const id of ['team-name','team-date','team-start-time','team-workout','team-notes','team-event','team-reps','team-rest'])if(typeof d.fields[id]==='string')el(id).value=d.fields[id];roster(d.athleteIds);globalThis.VXTUX?.restoreTargets(d.targets||{});el('team-save').textContent=editing?'Save team workout changes':'Assign workout';el('team-cancel').hidden=!editing;
 },discard:reset};
