...
(()=>{
 ...
 function history(openId){
  ...
  for(const w of [...data.teamWorkouts].sort((a,b)=>b.date.localeCompare(a.date))){
   ...
   const edit=document.createElement('button');edit.type='button';edit.textContent='Edit '+w.name;edit.addEventListener('click',()=>{editing=w.id;el('team-name').value=w.name;el('team-date').value=w.date;el('team-start-time').value=w.startTime||'';el('team-workout').value=w.workout;el('team-notes').value=w.notes;roster(w.assignments.map(a=>a.athleteId));el('team-save').textContent='Save team workout changes';el('team-cancel').hidden=false;status('Editing '+w.name+'. Changing the workout or date resets completion for all assigned athletes.');el('team-name').focus();});section.append(edit);
