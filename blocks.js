...
(()=>{
 ...
 function reset(){editing=null;draft.clear();el('block-plan').replaceChildren();el('block-form').reset();el('block-start').value=today();el('block-weeks').value='3';el('block-save').textContent='Save training block';el('block-cancel').hidden=true;athletes([]);plan();}
 function history(openId){
  ...
  for(const b of [...data.trainingBlocks].sort((a,b)=>b.startDate.localeCompare(a.startDate))){
   const section=document.createElement('details');section.className='session-summary';section.open=b.id===openId;
   ...
   const edit=document.createElement('button');edit.type='button';edit.textContent='Edit '+b.name;edit.addEventListener('click',()=>{
    editing=b.id;draft.clear();el('block-plan').replaceChildren();for(const w of b.plan)draft.set(w.week,{focus:w.focus,workouts:w.workouts});el('block-name').value=b.name;el('block-start').value=b.startDate;el('block-weeks').value=b.weeks;el('block-notes').value=b.notes;athletes(b.athleteIds);plan();el('block-save').textContent='Save block changes';el('block-cancel').hidden=false;status('Editing '+b.name);el('block-name').focus();
