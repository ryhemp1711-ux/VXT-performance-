...
function tab(id){document.querySelectorAll('.pane').forEach(p=>p.hidden=p.id!==id);document.querySelectorAll('nav button').forEach(b=>b.classList.toggle('active',b.dataset.tab===id));$('#page-title').textContent={dashboard:'Performance overview',results:'Results & personal bests',predictor:'Sprint-time predictor',backup:'Data & backup',cloud:'Cloud sync',screenshot:'Import screenshot',sessions:'Training sessions',blocks:'Training blocks'}[id];}
...
$('#roster').addEventListener('click',e=>{
 ...
 if(commit({...data,...(data.trainingBlocks?{trainingBlocks:data.trainingBlocks.map(b=>({...b,athleteIds:b.athleteIds.filter(id=>id!==a.id)}))}:{}),...(data.sessions?{sessions:data.sessions.map(s=>s.efforts?{...s,efforts:s.efforts.filter(e=>e.athleteId!==a.id)}:s)}:{}),athletes:data.athletes.filter(x=>x.id!==a.id),results:data.results.filter(x=>x.athleteId!==a.id),predictions:data.predictions.filter(x=>x.athleteId!==a.id)})){
